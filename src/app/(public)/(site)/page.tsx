...
import SplitedTitle from '../_components/splited-title';
import CircularTextButton from '../_components/circular-text-button';
...
export default async function Home() {
  ...
  return (
    <main>
      ...
      <section className="space-y-6 md:space-y-0 pt-6 md:pt-16 pb-14 md:pb-24 pl-6 md:pl-16">
        <header className="flex md:flex-row flex-col justify-between items-center md:items-baseline pr-6 md:pr-16 text-xl">
          <h1 className="z-10 md:h-24 lg:h-32">
            ...
            <span className="inline-block -ml-10 pl-10 font-script font-medium text-gradient text-7xl lg:text-9xl capitalize leading-[1.05]">
              hiện đại
            </span>
          </h1>
          <p className="max-w-[25ch] text-muted-foreground text-base lg:text-lg text-right tracking-wide">
            {hero.subtitle}
          </p>
        </header>
        <div className="after:top-0 after:right-0 after:absolute relative bg-secondary after:bg-gradient-to-l after:from-background after:to-background/0 rounded-ss-[5rem] after:w-16 after:h-full min-h-[250px] lg:min-h-[400px]">
          <SmoothImage
          ...
          <Link
            href={'/about'}
            className="group block right-16 lg:right-32 bottom-0 absolute w-fit translate-y-1/2"
          >
            <CircularTextButton
              text={hero.button}
              size={125}
      ...
      {/* Product section */}
      {featured.product && (
        <section className="gap-6 lg:gap-12 grid grid-cols-1 md:grid-cols-[1.6fr_2fr] p-6 md:p-16 py-12">
          <Link
            href={`/products/${featured.product.id}`}
            className="relative bg-secondary rounded-ss-[5rem] w-full aspect-square overflow-hidden"
          >
            <SmoothImage
          ...
          <div className="flex flex-col gap-6 md:gap-12">
            ...
            <Link
              className="self-stretch md:self-start btn-outline text-sm lg:text-base"
              href={`/products/${featured.product.id}`}
            >
              {featured.button}
      ...
      {/* Features section */}
      <section className="space-y-6 md:space-y-12 p-6 md:p-16 py-12">
        ...
        <div className="gap-4 grid bento">
          <article className="grid bg-secondary rounded-ss-[5rem]">
            <div className="space-y-2 p-6 lg:p-10 pb-2 lg:pb-4">
              <h3 className="font-semibold text-xl lg:text-2xl text-right">
                <SplitedTitle
              ...
              <p className="text-muted-foreground text-sm lg:text-base text-right">
          ...
          <article className="grid bg-secondary">
            <div className="z-10 relative space-y-2 p-6 lg:p-10 pb-2 lg:pb-4">
          ...
          <article className="grid grid-cols-2 bg-secondary">
            <div className="z-10 relative self-end space-y-2 -mr-24 p-6 lg:p-10">
              <h3 className="font-semibold text-xl lg:text-2xl">
                <SplitedTitle
      ...
      {/* Categories section */}
      {categories?.length && (
        <section className="flex flex-col gap-6 md:gap-12 p-6 md:p-16 py-12">
          <header className="flex md:flex-row flex-col justify-between items-center md:items-end gap-2">
            <h2 className="md:text-left text-center">
              <SplitedTitle
          ...
          <div className="gap-6 grid grid-cols-1 md:grid-cols-[2fr_1.5fr_1.5fr] auto-rows-[minmax(12rem,_1fr)] min-h-[450px]">
            {category.items.map((category) => (
              <Link
                ...
                key={category.id}
                className="group isolate relative bg-secondary first:rounded-ss-[5rem] overflow-hidden"
              >
                <SmoothImage
          ...
          <Link
            className="self-stretch md:self-center btn-outline text-sm lg:text-base"
            href={`/products/`}
          >
            {category.button}
            <span className="material-symbols-outlined">arrow_forward</span>
        ...
        <section className="space-y-6 md:space-y-12 p-6 md:p-16 py-12">
          <div className="gap-6 md:gap-12 grid grid-cols-1 md:grid-cols-2">
            <header className="flex flex-col items-center md:items-start gap-2 md:gap-4">
              ...
              <p className="max-w-[45ch] text-muted-foreground text-sm lg:text-base md:text-left text-center tracking-wide">
                {testimonials.subtitle}
              </p>
            </header>

            <div className="before:top-0 before:left-0 before:-z-10 before:absolute relative space-y-3 before:opacity-50 p-6 md:pr-0 before:font-extrabold md:before:text-[15rem] before:text-muted-foreground before:text-[10rem] before:content-['“'] before:leading-[0.6]">
          ...
          {testimonials.partners && testimonials.partners.length > 0 && (
            <div
              className="group relative h-12 overflow-hidden faded-sides"
              style={
                {
                  '--left': `${20}`,
                  '--right': `${80}`,
            ...
            >
              {testimonials.partners.map((src, index, array) => (
                <SmoothImage
                  ...
                  height={125}
                  alt=""
                  className="group-hover:paused -left-1/2 absolute grayscale hover:grayscale-0 w-auto h-full animate-slide cursor-pointer"
                  style={
                    {
                      '--speed': `${15}`,
        ...
        <section className="space-y-6 md:space-y-12 p-6 md:p-16 py-12">
          ...
          <div className="gap-6 grid grid-cols-1 md:grid-cols-[2fr_1fr]">
            <article className="gap-4 md:gap-6 grid grid-cols-2">
              <Link
                className="group relative col-span-2 bg-secondary rounded-ss-[5rem] overflow-hidden"
              ...
              <div className="flex flex-row md:flex-col gap-6 col-span-2 md:col-span-1">
                ...
                <Link
                  className="self-start btn-outline text-sm lg:text-base"
                  href={`/articles/${article.items[0].id}`}
                >
                  Đọc tiếp
            ...
            <div className="flex flex-col gap-6 bg-secondary p-6">
              ...
              <Link
                className="inline-flex items-center mt-auto text-muted-foreground hover:text-foreground text-sm lg:text-base transition-colors"
                href={'/articles'}
              >
                {article.button}
